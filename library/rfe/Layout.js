...
dojo.require('dojo.data.ItemFileWriteStore');   // needed for the grid until it understands the new dojo.store

dojo.require('rfe.StoreFileCache');
dojo.require('dijit.Tree');
dojo.require('rfe.dnd.Tree');
...
dojo.declare('rfe.Layout', null, {
	...
	constructor: function(args) {
		var storeMaster, storeMemory, storeGrid;
		...
		storeGrid = new dojo.data.ItemFileWriteStore({
			data: {
				identifier: 'id',
				items: []	// will be set on tree click
			}
		});
		this.storeCache = new rfe.StoreFileCache(storeMaster, storeMemory);
		this.grid = rfe.Grid.init('rfeGrid', storeGrid, this);
	...
	initTree: function(id, store) {
		...
		tree = new dijit.Tree({
			...
			singular: true // disallow selecting of multiple nodes in tree for dnd
		});
		tree.dndParams.push('singular');
		return tree;
	},
