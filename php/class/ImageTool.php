...
class ImageTool
{
    ...
    {
        $amount = $filterValues !== null ? $filterValues['amount'] : 30;
        $radius = $filterValues !== null ? $filterValues['radius'] : 0.9;
        $threshold = $filterValues !== null ? $filterValues['threshold'] : 0;
    ...
    public function createHistogram(&$gdImg, $calculateGray = false): array
    {
        $imgX = imagesx($gdImg);
        $imgY = imagesy($gdImg);
        $analysis = [
     ...
     * @return bool
     */
    function stretchHistogram(&$gdImg, $filterValues = null): bool
    {
        $band = $filterValues !== null ? $filterValues['band'] : '*';
        $method = $filterValues !== null ? $filterValues['method'] : 0;
        $threshold = $filterValues !== null ? $filterValues['threshold'] : 0.1;
     ...
     * @return array
     */
    public function getPixelColor(&$gdImg, $x, $y): array
    {
        return imagecolorsforindex($gdImg, imagecolorat($gdImg, $x, $y));
    }

    public function imageColorAllocateAlphaSafe(&$gdImgHexcolorAllocate, $R, $G, $B, $alpha = false): bool|int
    {
        if ($alpha) {
    ...
    public function getGrayscaleValue($r, $g, $b): float
    {
        return round(($r * 0.30) + ($g * 0.59) + ($b * 0.11));
    }

    public function getGrayscalePixel($OriginalPixel): array
    {
